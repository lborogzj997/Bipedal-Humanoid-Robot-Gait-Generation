...
#max_pitch_deg = 10
max_pitch_deg = 5
#A_y_base = 0.05  # Base link Y-axis movement amplitude
A_y_base = 0.11 # Base link Y-axis movement amplitude
#A_z_base = 0
A_z_base = 0.0
A_z_ratio = 1.0
...
arm_swing_deg = 20 # arm swing limit
arm_swing = np.radians(arm_swing_deg)  # Convert to radians
...
left_arm_swing = np.concatenate((
    arm_swing * np.sin(0.5 * np.pi * times1),
    arm_swing * np.cos(np.pi * (times2 - 1)),
    -arm_swing * np.cos(np.pi * (times3 - 2)),
    arm_swing * np.cos(np.pi * (times4 - 3))
))

right_arm_swing = np.concatenate((
    -arm_swing * np.sin(0.5 * np.pi * times1),
    -arm_swing * np.cos(np.pi * (times2 - 1)),
    arm_swing * np.cos(np.pi * (times3 - 2)),
    -arm_swing * np.cos(np.pi * (times4 - 3))
...
loop_left_arm_swing = np.concatenate((left_arm_swing[-len(times3)-len(times4):-len(times4)], left_arm_swing[-len(times4):]))
loop_right_arm_swing = np.concatenate((right_arm_swing[-len(times3)-len(times4):-len(times4)], right_arm_swing[-len(times4):]))
...
for _ in range(n_loops):
    ...
    left_arm_swing = np.concatenate((left_arm_swing, loop_left_arm_swing))
    right_arm_swing = np.concatenate((right_arm_swing, loop_right_arm_swing))
...
with open('foot_trajectory.csv', 'w', newline='') as csvfile:
    ...
    for i in range(len(times)):
        writer.writerow([
            ...
            left_arm_swing[i], right_arm_swing[i]
        ])
