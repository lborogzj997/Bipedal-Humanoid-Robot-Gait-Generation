...
# 找到左右肩关节索引
left_shoulder_pitch_index  = [i for i in range(p.getNumJoints(robot_id))
                                if p.getJointInfo(robot_id, i)[1].decode('utf-8') == 'left_shoulder_pitch_joint'][0]
right_shoulder_pitch_index = [i for i in range(p.getNumJoints(robot_id))
                                if p.getJointInfo(robot_id, i)[1].decode('utf-8') == 'right_shoulder_pitch_joint'][0]
...
for frame in frames:
    ...
    left_arm_pitch = frame[12]  # left_arm_swing
    right_arm_pitch = frame[13]  # right_arm_swing
    ...
    p.resetJointState(robot_id, left_shoulder_pitch_index, left_arm_pitch)
    p.resetJointState(robot_id, right_shoulder_pitch_index, right_arm_pitch)
